...
// retained until the domain is removed or the tracker is flushed. The first
// time we see an IP, conntrack entries with that orig-dst are flushed so
// existing direct connections reopen through the proxy.
func (t *Tracker) Track(ctx context.Context, domain, ip string) {
	t.mu.Lock()
	firstSeen := len(t.reverse[ip]) == 0
	...
	if firstSeen {
		if err := platform.RunSilent(ctx, "conntrack", "-D", "--orig-dst", ip); err != nil {
			t.logger.Debug("conntrack flush returned non-zero (likely no matches)", "ip", ip, "error", err)
		}
	}
}
...
func (t *Tracker) RemoveDomain(ctx context.Context, domain string) {
	...
	for _, ip := range toRemove {
		...
		// Drop live connections to this IP so the client reconnects directly
		// (no longer through the proxy) on next request.
		if err := platform.RunSilent(ctx, "conntrack", "-D", "--orig-dst", ip); err != nil {
			t.logger.Debug("conntrack flush returned non-zero (likely no matches)", "ip", ip, "error", err)
		}
	}
